...
public class RobotContainer implements Sendable {
  ...
  private BiConsumer<Double, Double> rrVelocitySetter =
      (Double x, Double y) -> {
        this.rrvx = x;
        this.rrvy = y;
      };
  ...
  private Supplier<SwerveRequest> driveCommand =
      () -> {
        ...
        if (Math.abs(rotationJoystickInput) > 0) {
          if (robotCentricNew) {
            return robotCentric
                .withVelocityX(driverController.getLeftY() * MaxSpeed * 0.2) // Drive forward with
                // negative Y (forward)
                .withVelocityY(
                    driverController.getLeftX() * MaxSpeed * 0.2) // Drive left with negative X
                // (left)
            ...
            desiredHeadingDeg =
                drivetrain.getState().Pose.getRotation().getDegrees(); // keep the desired heading
            // updated
            return drive
                .withVelocityX(
          ...
          return fieldCentricFacingAngle
              ...
              .withVelocityY(vy)
              .withTargetDirection(
                  Rotation2d.fromDegrees(desiredHeadingDeg)); // use the desired heading, in this
          // case controlled by vision
        } else if (rrvx > 0 || rrvy > 0) {
          return robotCentric.withVelocityX(rrvx).withVelocityY(rrvy);
        } else {
          if (robotCentricNew) {
            return robotCentric
                .withVelocityX(driverController.getLeftY() * MaxSpeed * 0.2) // Drive forward with
          ...
          return fieldCentricFacingAngle
              ...
              .withTargetDirection(
                  Rotation2d.fromDegrees(desiredHeadingDeg)); // use the desired heading, either
  ...
  private void
    ...
    driverController.povRight().onTrue(new InstantCommand(() -> this.desiredHeadingDeg += 30.0));

    driverController.povLeft().onTrue(new InstantCommand(() -> this.desiredHeadingDeg -= 30.0));
                    ...
                    new InstantCommand(
                        () -> {
                          ...
                          targetPoseFieldSpace =
                              robotPoseFieldSpace.plus(tagTransformRobotSpaceWpiConvention);
                          System.out.println(
                              "vector: x="
                    ...
                    new WaitUntilCommand(
                        () -> {
                          ...
                          velocitySetter.accept(
                              new Pair<Double, Double>(xOutputClamped, yOutputClamped));
