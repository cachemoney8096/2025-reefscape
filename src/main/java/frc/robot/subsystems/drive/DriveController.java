...
public class DriveController implements Sendable {
  ...
  public final ProfiledFieldCentricFacingAngle fieldCentricFacingAngle =
      new ProfiledFieldCentricFacingAngle(
              new TrapezoidProfile.Constraints(
                  maxAngularVelocityRadiansPerSecond, maxAngularVelocityRadiansPerSecond / 0.25))
          .withDriveRequestType(DriveRequestType.OpenLoopVoltage);
  public final ProfiledRobotCentricFacingAngle robotCentricFacingAngle =
      new ProfiledRobotCentricFacingAngle(
              new TrapezoidProfile.Constraints(
                  maxAngularVelocityRadiansPerSecond, maxAngularVelocityRadiansPerSecond / 0.25))
          .withDriveRequestType(DriveRequestType.OpenLoopVoltage);
  // public final RobotCentric robotController = new
  // RobotCentric().withDriveRequestType(DriveRequestType.OpenLoopVoltage);
        ...
        new RunCommand(
            () -> {
              if (robotRelativeActive) {
                new RunCommand(this::determineDesiredHeading)
                    .until(() -> !robotRelativeActive)
                    .schedule();
                drivetrain
                    .applyRequest(
                        () ->
                            robotCentricFacingAngle
                                .withVelocityX(
                                    -driverController.getLeftY() * MaxSpeed * this.getThrottle())
                                .withVelocityY(
                                    -driverController.getLeftX() * MaxSpeed * this.getThrottle())
                                .withTargetDirection(Rotation2d.fromDegrees(desiredHeading)))
                    .until(() -> !robotRelativeActive)
                    .schedule();
              } else {
                new RunCommand(this::determineDesiredHeading)
                    .until(getRobotRelativeActive)
                    .schedule();
                drivetrain
                    .applyRequest(
                        () ->
  ...
  public void determineDesiredHeading() {
    double input =
        JoystickUtil.squareAxis(MathUtil.applyDeadband(-driverController.getRightX(), 0.05)) * 6;
  ...
  public void initSendable(SendableBuilder builder) {
    builder.addDoubleProperty(
        "Gyro Rotation (deg)", (() -> gyro.getRotation2d().getDegrees()), null);
  }
}
