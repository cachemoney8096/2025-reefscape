...
public class ReefAngleCalcUtil {
  ...
  private static void init(ScoreTagPosition desiredTagScorePosition, boolean isRight) {
    ...
    tagScoreVectorMap.put(
        ...
        new Pair<Double, Double>(
            (scoreTagOffsetInches / 2) * Math.sqrt(3), (scoreTagOffsetInches / 2)));
    tagScoreVectorMap.put(
        ScoreTagPosition.ID_F, new Pair<Double, Double>((0.0), (scoreTagOffsetInches)));
    tagScoreVectorMap.put(
        ScoreTagPosition.ID_G,
        new Pair<Double, Double>(
            (scoreTagOffsetInches / 2) * Math.sqrt(3), (scoreTagOffsetInches / 2)));
  }
  ...
  public static Translation2d translateScorePositionOffset(
    ...
    Pair<Double, Double> calcVector =
        tagScoreVectorMap.get(ReefAngleCalcUtil.desiredTagScorePosition);
