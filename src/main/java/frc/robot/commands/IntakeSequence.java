...
public class IntakeSequence extends SequentialCommandGroup {
  ...
  public IntakeSequence(
      Claw claw, IntakeLimelight intakeLime, Arm arm, Elevator elevator, Climb climb) {
    ...
    addCommands(
        new ConditionalCommand(
            new SequentialCommandGroup(
                ...
                // the next two need to be done at the same time as the driving
                moveArmElevatorClaw),
            moveArmElevatorClaw, // return out of the command if the robot does not see the intake
            // april tag
            () -> {
              return (!isTag.isEmpty()
                  && ((tagid == 1) || (tagid == 2) || (tagid == 12) || (tagid == 13)));
            }));
  }
}
