...
public class TestDrive extends SequentialCommandGroup {
  public TestDrive(
    ...
    Pose2d tagPoseFieldSpace =
        new Pose2d(
            tagPose3dRobotSpace.getZ() * headingRot.getCos()
                - tagPose3dRobotSpace.getX() * headingRot.getSin(),
            tagPose3dRobotSpace.getZ() * headingRot.getSin()
                + tagPose3dRobotSpace.getX() * headingRot.getCos(),
            Rotation2d.fromDegrees(targetHeading));
    Pose2d botPoseFieldSpaceAtReading = drivetrain.getState().Pose;
    List<Waypoint> waypoints =
        PathPlannerPath.waypointsFromPoses(botPoseFieldSpaceAtReading, tagPoseFieldSpace);
    PathConstraints constraints = new PathConstraints(0.3, 0.3, 0.3, 0.3);
    PathPlannerPath path =
        new PathPlannerPath(
            waypoints, constraints, null, new GoalEndState(0.0, tagPoseFieldSpace.getRotation()));
    addCommands(
